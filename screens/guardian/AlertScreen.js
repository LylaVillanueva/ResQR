...
export default function AlertScreen({ navigation }) {
  const { alerts, users } = useAdminData();
          ...
          filteredAlerts.map((alert) => (
            <View key={alert.id} style={[styles.alertCard, alert.status === 'escalated' && styles.alertCardEscalated]}>
              ...
              <View style={styles.statusRow}>
                <Text style={[styles.statusText, confirmationStyle(alert.guardianStatus), styles.shadow]}>
                  Guardian: {alert.guardianStatus}
                </Text>
                <Text style={[styles.statusText, confirmationStyle(alert.responderStatus), styles.shadow]}>
                  Responder: {alert.responderStatus}
                </Text>
              </View>
...
const styles = StyleSheet.create({
  ...
  detailsButton: { flexDirection: 'row', justifyContent: 'space-between', alignItems: 'center', borderRadius: 10, borderWidth: 1, borderColor: '#245490', paddingVertical: 7, paddingHorizontal: 16, backgroundColor: '#d3e5f8' },
  detailsText: { fontSize: 14, fontFamily: 'Poppins_500Medium', color: '#245490' },
