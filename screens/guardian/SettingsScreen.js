...
export default function SettingsScreen({ navigation, session, setSession }) {
  // Defaults on — missing a ward's emergency alert is the one thing this
  // app exists to prevent.
  const [notifications, setNotifications] = useState({ push: true, alertSound: true });
  ...
  function handleNotificationToggle(key, label, nextValue) {
    ...
    Alert.alert(
      `Turn off "${label}"?`,
      'You may miss your ward\'s emergency alert if this is off. Are you sure?',
      [
