...
export default function AlertScreen({ navigation }) {
  ...
  const myAlerts = useMemo(
    () => alerts.filter((alert) => alert.responderId === account.id),
    [alerts, account.id]
  );
        ...
        {filteredAlerts.length === 0 ? (
          ...
          <View key={alert.id} style={[styles.alertCard, alert.status === 'escalated' && styles.alertCardEscalated]}>
            ...
            <View style={styles.statusRow}>
              <Text style={[styles.statusText, confirmationStyle(alert.guardianStatus)]}>Guardian: {'\n'}{alert.guardianStatus}</Text>
              <Text style={[styles.statusText, confirmationStyle(alert.responderStatus)]}>Responder: {'\n'}{alert.responderStatus}</Text>
            </View>
...
const styles = StyleSheet.create({
  ...
  heading1: { fontSize: typography.section, fontFamily: 'Poppins_600SemiBold', marginLeft: 0, marginBottom: 6, marginTop: 8},
  ...
  auditButton: { alignItems: 'center', padding: 5 },
  auditButtonText: { fontSize: typography.caption, fontFamily: 'Poppins_500Medium', color: '#666', marginTop: 2 },
  ...
  detailsButton: {
   ...
   minHeight: 48, ...alertLayout.details },
  detailsText: {
